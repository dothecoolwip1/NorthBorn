...
function testUser() {
  ...
  return {
    ...
    confirmed_at: now(),
    last_sign_in_at: now(),
    app_metadata: { provider: 'email', providers: ['email'], northborn_test: true },
    user_metadata: { full_name: `${user.label} Test`, northborn_test: true },
    identities: [],
    created_at: now(),
    updated_at: now(),
    is_anonymous: false,
...
function testSession() {
  return {
    access_token: 'northborn-test-token',
    refresh_token: 'northborn-test-refresh',
    expires_in: 3600,
...
function readGeneric(table:string) {
  try { return JSON.parse(localStorage.getItem(`${GENERIC_PREFIX}${table}`) || '[]') as any[] } catch { return [] }
}
function writeGeneric(table:string, rows:any[]) {
  localStorage.setItem(`${GENERIC_PREFIX}${table}`, JSON.stringify(rows))
...
function baseRows(table:string): any[] {
  ...
  const managerMembership = { id:'test-manager-membership', organization_id:TEST_ORG.id, user_id:TEST_USERS.manager.id, status:'active', organization:{ id:TEST_ORG.id, name:TEST_ORG.name } }
  const operatorMembership = { id:'test-operator-membership', organization_id:TEST_ORG.id, user_id:TEST_USERS.operator.id, status:'active', organization:{ id:TEST_ORG.id, name:TEST_ORG.name } }
  switch(table) {
...
function saveRows(table:string, rows:any[]) {
  ...
  if (['organizations','organization_members','roles','membership_roles','customer_portal_users'].includes(table)) return
...
class TestQuery implements PromiseLike<any> {
  ...
  upsert(payload:any,_options?:any){this.mode='insert';this.payload=payload;return this}
  ...
  in(key:string,value:any[]){this.filters.push({op:'in',key,value});return this}
  is(key:string,value:any){this.filters.push({op:'is',key,value});return this}
  order(key:string,options?:{ascending?:boolean}){this.orderKey=key;this.orderAscending=options?.ascending!==false;return this}
  ...
  private async execute(single=false,strict=false){
    try{
      let rows=baseRows(this.table)
      const matched=rows.filter(r=>this.matches(r))
      if(this.mode==='insert'){
        const incoming=Array.isArray(this.payload)?this.payload:[this.payload]
        const added=incoming.map((item:any)=>({ id:item.id||uid(), created_at:item.created_at||now(), updated_at:item.updated_at||now(), ...item }))
        if(this.table==='employee_fleet_access_profiles'){
          for(const item of added){const index=rows.findIndex(r=>r.organization_id===item.organization_id&&r.employee_id===item.employee_id);if(index>=0)rows[index]={...rows[index],...item};else rows.push(item)}
        }else rows=[...rows,...added]
        saveRows(this.table,rows)
        const data=added
        return { data:single?(data[0]||null):data, error:null }
      }
      if(this.mode==='update'){
        rows=rows.map(row=>this.matches(row)?{...row,...this.payload,updated_at:now()}:row)
        saveRows(this.table,rows)
        const data=rows.filter(r=>this.matches(r))
        return { data:single?(data[0]||null):data, error:null }
      }
      if(this.mode==='delete'){
      ...
      let data=[...matched]
      if(this.orderKey){const key=this.orderKey,dir=this.orderAscending?1:-1;data.sort((a,b)=>String(a?.[key]??'').localeCompare(String(b?.[key]??''))*dir)}
      if(this.limitCount!==null)data=data.slice(0,this.limitCount)
      if(single){
        if(strict&&!data.length)return {data:null,error:{message:'No rows found'}}
...
function operatorNameForJob(jobId:string){
  const data=readTestLabData();const employeeIds=data.assignments.filter(a=>a.job_id===jobId&&a.employee_id).map(a=>a.employee_id);const operator=data.employees.find(e=>employeeIds.includes(e.id)&&e.position?.toLowerCase()==='operator')||data.employees.find(e=>employeeIds.includes(e.id));return operator||null
}
function portalJobs(customerId:string){
  const data=readTestLabData();const contacts=readTestClientContacts();const meta=readJobMeta()
  return data.jobs.filter(j=>j.customer_id===customerId).map(j=>{
    const operator=operatorNameForJob(j.id);const m=meta[j.id]||{};const contact=contacts.find(c=>c.id===m.contact_id)||null
    return {job_id:j.id,job_number:j.job_number,title:j.title,site_name:j.site_name,site_address:j.site_address,scheduled_start:j.onsite_time||j.scheduled_start,scheduled_end:j.scheduled_end,status:j.status,completed_at:j.completed_at||null,onsite_contact_id:contact?.id||null,onsite_contact_name:contact?.name||null,onsite_contact_title:contact?.title||null,onsite_contact_phone:contact?.phone||null,onsite_contact_email:contact?.email||null,operator_name:operator?`${operator.first_name} ${operator.last_name}`:null,operator_phone:operator?.phone||null,dispatch_phone:'403-555-0101',emergency_phone:'403-555-0191',client_notes:m.notes||null}
  })
}
...
async function fakeRpc(name:string,args:any={}){
  ...
  switch(name){
    ...
    case 'update_my_customer_portal_company': {
      const customers=data.customers.map(c=>c.id===args._customer_id?{...c,name:args._name||c.name,phone:args._phone||null,address:args._address||null,billing_email:args._billing_email||null}:c);writeTestLabData({...data,customers});return {data:true,error:null}
    }
    case 'upsert_my_customer_contact': {
      const contacts=readTestClientContacts();const id=args._contact_id||uid();const next={id,name:args._name,title:args._title||null,phone:args._phone||null,email:args._email||null,contact_type:args._contact_type,status:'active',updated_at:now()};const index=contacts.findIndex(c=>c.id===id);if(index>=0)contacts[index]=next;else contacts.push(next);writeTestClientContacts(contacts);return {data:id,error:null}
    }
    ...
    case 'get_my_assigned_job_contacts': {
...
function restore(){
  ...
  client.functions.invoke=original.functionsInvoke
  client.auth.getSession=original.authGetSession
  client.auth.getUser=original.authGetUser
  client.auth.onAuthStateChange=original.authOnAuthStateChange
  client.auth.signOut=original.authSignOut
}

export default function TestSupabaseBridge({persona,children}:{persona:TestPersona;children:ReactNode}){
  activePersona=persona
  install(persona)
  useEffect(()=>{activePersona=persona;return()=>{}},[persona])
  useEffect(()=>()=>restore(),[])
  return <>{children}</>
}
