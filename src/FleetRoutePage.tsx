...
export default function FleetRoutePage(){
  const [session,setSession]=useState<Session|null>(null),[operator,setOperator]=useState<OperatorContext|null>(null),[loading,setLoading]=useState(true),[manager,setManager]=useState(false),[roleKey,setRoleKey]=useState('')
  useEffect(()=>{
    let active=true
    const load=async()=>{
      if(localStorage.getItem(TEST_MODE_KEY)==='1'){if(active){setManager(true);setRoleKey('owner');setLoading(false)};return}
