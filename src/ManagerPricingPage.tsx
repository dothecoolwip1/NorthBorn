...
export default function ManagerPricingPage(){
  ...
  return <div className="pricing-shell">
    ...
    <main className="pricing-page">
      ...
      <section className="pricing-mode-card"><label><span>Pricing for</span><select value={customerId} onChange={e=>chooseCustomer(e.target.value)}><option value="">Standard company price sheet</option>{customers.map(c=><option value={c.id} key={c.id}>{c.name}</option>)}</select></label>{selectedCustomer&&<p>You are editing client-specific rates for <strong>{selectedCustomer.name}</strong>. Blank overrides use the standard rate.</p>}</section>

      {!selectedCustomer&&canManage&&<form className="pricing-add" onSubmit={addItem}><div className="pricing-section-head"><div><strong>Add item</strong><span>Equipment, labour, disposal, material or another billable service.</span></div><button type="button" className="pricing-secondary" onClick={()=>void addCommon()} disabled={busy}>Add common items</button></div><div className="pricing-add-grid"><input placeholder="Item name" value={newItem.name} onChange={e=>setNewItem({...newItem,name:e.target.value})} required/><select value={newItem.category} onChange={e=>setNewItem({...newItem,category:e.target.value})}>{CATEGORIES.map(c=><option value={c} key={c}>{label(c)}</option>)}</select><select value={newItem.unit} onChange={e=>setNewItem({...newItem,unit:e.target.value})}>{UNITS.map(u=><option value={u} key={u}>{u}</option>)}</select><input type="number" min="0" step="0.01" placeholder="Rate" value={newItem.rate} onChange={e=>setNewItem({...newItem,rate:e.target.value})}/><button className="pricing-primary" disabled={busy}><Plus size={17}/>Add</button></div></form>}

      <section className="pricing-list-section"><div className="pricing-section-head"><div><strong>{selectedCustomer?'Client rates':'Standard rates'}</strong><span>{items.length} price sheet item{items.length===1?'':'s'}</span></div></div><div className="pricing-list">{items.map(item=>{const d=drafts[item.id]||{name:item.name,category:item.category,unit:item.unit,rate:String(item.default_rate)};const override=overrideMap.get(item.id);return <article className="pricing-row" key={item.id}><div className="pricing-row-title"><strong>{item.name}</strong><span>{label(item.category)} · per {item.unit}</span></div>{selectedCustomer?<><div className="pricing-standard"><small>Standard</small><b>{money(item.default_rate)}</b></div><label className="pricing-rate"><span>{selectedCustomer.name} rate</span><input key={`${item.id}-${override?.id||'std'}-${override?.rate??''}`} defaultValue={override?String(override.rate):''} placeholder={money(item.default_rate)} type="number" min="0" step="0.01" onBlur={e=>void saveOverride(item,e.currentTarget.value)}/></label>{override&&<button className="pricing-icon" title="Use standard rate" onClick={()=>void saveOverride(item,'')} disabled={busy}><RotateCcw size={17}/></button>}</>:<><div className="pricing-fields"><input value={d.name} onChange={e=>setDrafts({...drafts,[item.id]:{...d,name:e.target.value}})}/><select value={d.category} onChange={e=>setDrafts({...drafts,[item.id]:{...d,category:e.target.value}})}>{CATEGORIES.map(c=><option value={c} key={c}>{label(c)}</option>)}</select><select value={d.unit} onChange={e=>setDrafts({...drafts,[item.id]:{...d,unit:e.target.value}})}>{UNITS.map(u=><option value={u} key={u}>{u}</option>)}</select><input type="number" min="0" step="0.01" value={d.rate} onChange={e=>setDrafts({...drafts,[item.id]:{...d,rate:e.target.value}})}/></div><div className="pricing-row-actions"><button className="pricing-icon save" title="Save" onClick={()=>void saveItem(item)} disabled={busy}><Save size={17}/></button><button className="pricing-icon danger" title="Remove" onClick={()=>void removeItem(item)} disabled={busy}><Trash2 size={17}/></button></div></>}</article>})}{!items.length&&<div className="pricing-empty">No price sheet items yet. Add your first service above.</div>}</div></section>
    </main>
  </div>
}
