...
async function loadWorkspace(userId: string): Promise<Workspace | null> {
  const { data: membership, error } = await db.from('organization_members').select('id,organization_id,organization:organizations(name)').eq('user_id', userId).eq('status', 'active').limit(1).maybeSingle()
...
export default function TeamAccessPage() {
  ...
  const refresh = useCallback(async (activeWorkspace: Workspace) => {
    ...
    const { data: memberRows, error: membersError } = await db.from('organization_members').select('id,user_id,status,joined_at').eq('organization_id', orgId).order('joined_at', { ascending: true })
  ...
  useEffect(() => {
    ...
    const start = async () => {
      ...
      } catch (err) { if (active) setError(readError(err)) }
  ...
  const createInvite = async (event: React.FormEvent) => {
    ...
    if (!workspace) return
    setBusy(true);setError('');setDeliveryMessage('');setInviteLink('');setCopied(false)
    try {
      const normalizedEmail = email.trim().toLowerCase()
      const selectedRole = TEAM_ROLES.find(role => role.key === roleKey)
      const { data, error: functionError } = await supabase.functions.invoke('send-team-invite', { body:{ organizationId:workspace.organizationId,organizationName:workspace.organizationName,email:normalizedEmail,roleKey,roleName:selectedRole?.label||roleKey } })
      ...
      setDeliveryMessage(data.emailSent ? `Invitation email sent to ${normalizedEmail}.` : `Invitation created, but the email could not be sent yet: ${data.deliveryError || 'email delivery is not configured'}. You can still copy the invite link below.`)
      setEmail('')
  ...
  const copyInvite = async () => { if (!inviteLink) return; await navigator.clipboard.writeText(inviteLink); setCopied(true); window.setTimeout(() => setCopied(false), 1800) }
  const revokeInvite = async (inviteId: string) => { if (!workspace) return; setBusy(true);setError('');try{const { error: rpcError }=await db.rpc('revoke_organization_invite',{_invite_id:inviteId});if(rpcError)throw rpcError;await refresh(workspace)}catch(err){setError(readError(err))}finally{setBusy(false)} }

  if (loading) return <div className="team-page team-center">Loading team access…</div>
  if (!session) return <div className="team-page team-center"><div className="team-card compact-card"><ShieldCheck size={36}/><h1>Sign in required</h1><p>Sign in to Northborn before managing company access.</p><NavLink className="team-primary" to="/">Go to sign in</NavLink></div></div>
  if (!workspace) return <div className="team-page team-center"><div className="team-card compact-card"><h1>No company found</h1><p>This account is not attached to a Northborn company yet.</p><NavLink className="team-primary" to="/">Return to Northborn</NavLink></div></div>
  if (!canManage) return <div className="team-page team-center"><div className="team-card compact-card"><ShieldCheck size={36}/><h1>Owner or admin access required</h1><p>Your current role is {workspace.roleKey}.</p><NavLink className="team-primary" to="/">Return to Northborn</NavLink></div></div>

  return <div className="team-page"><div className="team-wrap">
    <div className="team-topbar"><NavLink to="/" className="team-back"><ArrowLeft size={18}/>Northborn</NavLink><span>{workspace.organizationName}</span></div>
