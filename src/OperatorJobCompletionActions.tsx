...
function categoryFor(description:string){
  if(description==='Swamper')return 'labour'
  if(description==='Disposal')return 'disposal'
  if(description==='Overtime')return 'overtime'
  if(description==='Crew Truck')return 'transport'
  if(description==='Other')return 'other'
  return 'equipment'
}
function newLine(description=''):Line{return {id:uid(),category:categoryFor(description),description:description==='Other'?'':description,quantity:'1',unit:'hour',rate:''}}
...
export default function OperatorJobCompletionActions({job,organizationId,organizationName,onCompleted}:Props){
  ...
  const totals=useMemo(()=>{
    const subtotal=lines.reduce((sum,line)=>sum+asNumber(line.quantity)*asNumber(line.rate),0)
    const tax=subtotal*(asNumber(form.tax_rate)/100)
  ...
  const completeJob=async()=>{
    ...
    try{
      if(testMode){
        const data=JSON.parse(localStorage.getItem(TEST_DATA_KEY)||'{}') as {jobs?:Array<Record<string,unknown>>}
        const jobs=(data.jobs||[]).map(item=>item.id===job.id?{...item,status:'completed',completed_at:new Date().toISOString(),completed_by:'test-operator'}:item)
        localStorage.setItem(TEST_DATA_KEY,JSON.stringify({...data,jobs}))
        window.dispatchEvent(new Event('northborn-test-data-changed'))
      }else{
        const result=await db.rpc('complete_my_assigned_job',{_organization_id:organizationId,_job_id:job.id})
    ...
    try{
      if(testMode){
        ...
        if(existing){
          ...
          setForm({purchase_order:existing.purchase_order||'',afe_number:existing.afe_number||'',project:existing.project||job.title,location:existing.location||job.site_address||job.site_name||'',area:existing.area||'',job_description:existing.job_description||job.title,authorization_date:existing.authorization_date||'',authorized_by_name:existing.authorized_by_name||'',authorization_contact:existing.authorization_contact||'',authorization_email:existing.authorization_email||'',tax_rate:String(existing.tax_rate??5),notes:existing.notes||''})
          setLines((existing.line_items||[]).map((line:any)=>({id:line.id||uid(),category:line.category||'other',description:line.description||'',quantity:String(line.quantity??1),unit:line.unit||'hour',rate:String(line.rate??0)})))
        }else{
          setInvoiceNumber('')
          setForm(defaultForm(job));setLines([newLine()])
        }
      }else{
        const result=await db.rpc('get_my_assigned_job_invoice_draft',{_organization_id:organizationId,_job_id:job.id})
        if(result.error)throw result.error
        const invoice=result.data?.invoice
        if(invoice){
          setInvoiceNumber(invoice.invoice_number||'')
          setForm({purchase_order:invoice.purchase_order||'',afe_number:invoice.afe_number||'',project:invoice.project||job.title,location:invoice.location||job.site_address||job.site_name||'',area:invoice.area||'',job_description:invoice.job_description||job.title,authorization_date:invoice.authorization_date||'',authorized_by_name:invoice.authorized_by_name||'',authorization_contact:invoice.authorization_contact||'',authorization_email:invoice.authorization_email||'',tax_rate:String(invoice.tax_rate??5),notes:invoice.notes||''})
          setLines((result.data?.line_items||[]).map((line:any)=>({id:line.id||uid(),category:line.category||'other',description:line.description||'',quantity:String(line.quantity??1),unit:line.unit||'hour',rate:String(line.rate??0)})))
        }else{
          setInvoiceNumber('');setForm(defaultForm(job));setLines([newLine()])
        }
      }
  ...
  const saveDraft=async()=>{
    ...
    try{
      ...
      if(testMode){
        ...
        localStorage.setItem(TEST_INVOICE_KEY,JSON.stringify(invoices))
      ...
      setNotice('Draft saved. Manager or accounting can review and issue it.')
    }catch(e){setError(readError(e))}finally{setBusy(false)}
  }
  ...
  return <div className="operator-completion-actions">
    ...
    {invoiceOpen&&<div className="operator-invoice-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget&&!busy)setInvoiceOpen(false)}}><section className="operator-invoice-modal" role="dialog" aria-modal="true">
      ...
      {loading?<div className="operator-invoice-loading">Loading invoice draft…</div>:<>
        ...
        {error&&<div className="operator-action-error">{error}</div>}{notice&&<div className="operator-action-success">{notice}</div>}
        <div className="operator-invoice-grid">
          <label><span>P.O. #</span><input value={form.purchase_order} onChange={e=>setForm({...form,purchase_order:e.target.value})}/></label>
          <label><span>A.F.E. #</span><input value={form.afe_number} onChange={e=>setForm({...form,afe_number:e.target.value})}/></label>
          <label><span>Project</span><input value={form.project} onChange={e=>setForm({...form,project:e.target.value})}/></label>
          <label><span>Area</span><input value={form.area} onChange={e=>setForm({...form,area:e.target.value})}/></label>
          <label className="wide"><span>Location</span><input value={form.location} onChange={e=>setForm({...form,location:e.target.value})}/></label>
          <label className="wide"><span>Work completed</span><textarea value={form.job_description} onChange={e=>setForm({...form,job_description:e.target.value})}/></label>
        </div>
        <div className="operator-invoice-section"><div className="operator-invoice-section-head"><div><strong>Billable items</strong><span>Add equipment, labour, disposal and other charges.</span></div><button type="button" onClick={()=>setLines([...lines,newLine()])}><Plus size={16}/>Line</button></div><div className="operator-preset-row">{PRESETS.map(preset=><button type="button" key={preset} onClick={()=>setLines([...lines,newLine(preset)])}>{preset}</button>)}</div><div className="operator-line-list">{lines.map((line,index)=><div className="operator-line" key={line.id}><input className="description" placeholder="Description" value={line.description} onChange={e=>setLines(lines.map((item,i)=>i===index?{...item,description:e.target.value}:item))}/><input type="number" min="0" step="0.01" placeholder="Qty" value={line.quantity} onChange={e=>setLines(lines.map((item,i)=>i===index?{...item,quantity:e.target.value}:item))}/><select value={line.unit} onChange={e=>setLines(lines.map((item,i)=>i===index?{...item,unit:e.target.value}:item))}>{UNITS.map(unit=><option value={unit} key={unit}>{unit}</option>)}</select><input type="number" min="0" step="0.01" placeholder="Rate" value={line.rate} onChange={e=>setLines(lines.map((item,i)=>i===index?{...item,rate:e.target.value}:item))}/><strong>{money(asNumber(line.quantity)*asNumber(line.rate))}</strong><button type="button" className="delete" onClick={()=>setLines(lines.filter((_,i)=>i!==index))}><Trash2 size={16}/></button></div>)}</div></div>
